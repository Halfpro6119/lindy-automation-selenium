...
class LindyAutomationPlaywright:
    ...
    async def setup(self, use_saved_session=True):
        ...
        self.browser = await self.playwright.chromium.launch(
            headless=False,  # Changed to False for visible browser
            args=[
                '--no-sandbox',
    ...
    async def run(self):
        ...
        try:
            ...
            # Don't cleanup - keep browser open so you can see
            print("\nBrowser will remain open so you can see the results.")
            print("Press Ctrl+C to close when done.")
            await asyncio.sleep(3600)  # Wait 1 hour before closing
