"""
Lindy Automation Script - Full Playwright Version
Runs the entire automation using Playwright with visible browser
"""
...
class LindyAutomation:
    ...
    async def setup(self):
        ...
        # Launch browser in HEADED mode (visible) using chromium
    ...
    async def login_to_google(self):
        ...
        try:
            ...
            google_btn = await self.page.query_selector("button:has-text('Google'), button:has-text('Continue with Google'), a:has-text('Sign in with Google')")
            if google_btn:
    ...
    async def fill_signup_form(self):
        ...
        try:
            ...
            # This is a generic approach - adjust based on actual form
            ...
            continue_btn = await self.page.query_selector("button:has-text('Continue'), button:has-text('Next'), button:has-text('Get Started')")
            if continue_btn:
    ...
    async def start_free_trial(self):
        ...
        try:
            ...
            trial_btn = await self.page.query_selector("button:has-text('Start Free Trial'), button:has-text('Free Trial'), a:has-text('Start Trial')")
            ...
            # Click Save Card
            save_btn = await self.page.query_selector("button:has-text('Save Card'), button:has-text('Save'), button:has-text('Submit')")
            if save_btn:
...
async def main():
    """Main entry point"""
    print("\n" + "="*70)
    print("Lindy Automation - Full Playwright Version")
    print("Running with visible browser so you can see everything")
    print("="*70 + "\n")
